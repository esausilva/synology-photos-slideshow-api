...
public class SynologyApiInfoProviderTests
{
    ...
    private static (SynologyApiInfoProvider provider, ISynologyApiClient apiClient) CreateProvider()
    {
        var apiClient = Substitute.For<ISynologyApiClient>();
        var apiInfoApi = Substitute.For<IApiInfoClient>();

        apiClient.ApiInfoApi.Returns(apiInfoApi);

        apiInfoApi
            .QueryAsync(Arg.Any<ApiInfoRequest>(), Arg.Any<CancellationToken>())
        ...
        return (new SynologyApiInfoProvider(apiClient), apiClient);
    }
